...
impl Decimal {
    ...
    pub fn format<const N: usize>(&self, output: &mut StaticVec<u8, N>) {
        format_big_int::<192, { Self::SCALE }, N>(&self.0, output);
    }
...
impl TryFrom<&[u8]> for Decimal {
    ...
    fn try_from(value: &[u8]) -> Result<Self, BigIntError> {
        Ok(Self(BigInt::<192>::from_bytes(value)?))
    }
}
...
mod tests {
    ...
    impl fmt::Display for Decimal {
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> Result<(), fmt::Error> {
            ...
            format_big_int::<192, { Self::SCALE }, { Self::MAX_PRINT_LEN }>(
                &self.0,
                &mut formatted,
            );
    ...
    #[test]
    pub fn test_decimal_size() {
        assert_eq!(Decimal::SIZE_IN_BYTES, 24);
    }
    ...
    pub fn test_format_decimal() {
        ...
        assert_eq!(Decimal(BigInt::<192>::from_bytes(&[
            0x00, 0x00, 0x78, 0x62, 0xa4, 0x41, 0xa7, 0x10,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ]).unwrap()).to_string(), "1.2");
        assert_eq!(Decimal::MAX.to_string(), "3138550867693340381917894711603833208051.177722232017256447");
        assert_eq!(Decimal::MIN.is_negative(), true);
        assert_eq!(Decimal::MIN.to_string(), "-3138550867693340381917894711603833208051.177722232017256448");
    }

    const DECIMAL_1: [u8; 26] = [
        0x5c, 0xa0,
        0x00, 0x00, 0x64, 0xa7, 0xb3, 0xb6, 0xe0, 0x0d, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ];
    const DECIMAL_10: [u8; 26] = [
        0x5c, 0xa0,
        0x00, 0x00, 0xe8, 0x89, 0x04, 0x23, 0xc7, 0x8a, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ];
    const DECIMAL_12: [u8; 26] = [
        0x5c, 0xa0,
        0x00, 0x00, 0xb0, 0xd8, 0x6b, 0x90, 0x88, 0xa6, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ];
    const DECIMAL_123: [u8; 26] = [
        0x5c, 0xa0,
        0x00, 0x00, 0x0c, 0x6d, 0x51, 0xc8, 0xf7, 0xaa, 0x06, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ];
    const DECIMAL_1_23456789: [u8; 26] = [
        0x5c, 0xa0,
        0x00, 0xb4, 0x8d, 0x76, 0xf4, 0x10, 0x22, 0x11, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ];
    const DECIMAL_0_23456789: [u8; 26] = [
        0x5c, 0xa0,
        0x00, 0xb4, 0x29, 0xcf, 0x40, 0x5a, 0x41, 0x03, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, ];
