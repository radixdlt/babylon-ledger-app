...
/// - Flip flop: Used for handling key/value pairs in maps
/// - Phase pointer: pointer in the decoding phases array specific for particular type we're decoding at this nesting level
/// (see `type_info.rs` for type-specific decoding steps)
#[repr(transparent)]
...
/// Decoder state for each nesting level
#[repr(C, packed)]
#[derive(Copy, Clone, Debug)]
struct State {
...
/// After processing each byte decoder informs if the input is completely processed or we need
/// to feed more data. Each state returns the number of bytes already processed.
...
/// Events generated by the decoder
#[derive(Copy, Clone, Debug, PartialEq)]
pub enum SborEvent {
    Start {
...
/// Streaming decoder for SBOR format
/// Note that fields are ordered to minimize the size of the struct.
#[repr(C, packed)]
...
impl SborDecoder {
    ...
    /// SBOR uses variable length encoding for lengths. This function reads the length bytes
    /// and decodes them into plain value.
